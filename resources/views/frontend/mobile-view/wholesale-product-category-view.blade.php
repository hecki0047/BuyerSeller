...
<div class="container">
 ...
 @foreach($wholesale_product as $data)
	<div class="col-xs-12 padding_0" style="margin-bottom:2%;background: #fff;margin-top:2%;">
			<a href="{{ URL::to('product-details/'.preg_replace('/[^A-Za-z0-9\.-]/', '-',$data->category_product_name->name).'='.mt_rand(100000000, 999999999).$data->product_id,null,is_secured()) }}">
				<div class="cat_pro_list">
                    ...
                    @if(count([$pro_img_new]) == 0)
                        <img class="cat_pro_list_img img-responsive" src="{{ URL::to((isset($pro_img->image)) ? $pro_img->image : 'no_image.jpg',is_secured()) }}" alt="" />
                    @else
                       
                        @if($data->pro_parent_cat && $data->bdtdcCategory)
                        <img class="cat_pro_list_img img-responsive" src="{{ URL::to((isset($pro_img_new->image)) ? $pro_img_new->image : 'no_image.jpg',is_secured()) }}" alt="" />
                        @else
                        <img class="cat_pro_list_img img-responsive" src="{{ URL::to('uploads/no-image.jpg',is_secured()) }}" alt="" />
                        @endif
		...
		<div class="cat_pro_list_des">
    		...
    		<div style="display: block;width: 100%; margin:0; padding: 0; clear: both; padding-top: 10px;">
    			<div>
    				<div class="pro_sur"><img style="height:18px;width:18px; float: left;" src="{!! asset('assets/images/Buyer-protection-home.png',is_secured()) !!}">
    				</div>
                    ...
                    @if($data)
                        @if($data->supp_pro_company)
                            ...
                            <img style="height:16px;width:24px; display: block;clear: both;" src="{{ asset('assets/global/img/flags/'.strtolower($data->supp_pro_company->location_of_reg_string->iso_code_2).'.png',is_secured())}}" alt="{{ $data->supp_pro_company->location_of_reg_string->name }}" >
                            </span>
                            @endif
                        @endif
                    @endif 
