									...
									@foreach($supplier_products as $sps)
										<li title="{{$sps->product_name?$sps->product_name->name:'not available'}}" data-pid="{{$sps->id}}">
											...
											<div class="product-product-name">
													<span style="font-size: 11px;"><a target="_blank" href="{{ URL::to('product-details/'.preg_replace('/[^A-Za-z0-9\.-]/', '-',($sps->product_name?$sps->product_name->name:'not available')).'='.mt_rand(100000000, 999999999).$sps->id,null,is_secured()) }}">{{$sps->product_name?$sps->product_name->name:'not available'}}</a></span>
											</div>
										</li>
									@endforeach
									@endif
								...
								<div class="product-select-body">
									...
									@foreach($spg->BdtdcSupplierProductGroupsProducts as $sgp)
										<li title="{{$sgp->product_name?$sgp->product_name->name:'not available'}}" data-pid="{{$sgp->id}}">
											...
											<div class="product-product-name">
													<span style="font-size: 11px;"><a target="_blank" href="{{ URL::to('product-details/'.preg_replace('/[^A-Za-z0-9\.-]/', '-',($sgp->product_name?$sgp->product_name->name:'not available')).'='.mt_rand(100000000, 999999999).$sgp->id,null,is_secured()) }}">{{$sgp->product_name?$sgp->product_name->name:'not available'}}</a></span>
											</div>
										</li>
									@endforeach
									@endif
