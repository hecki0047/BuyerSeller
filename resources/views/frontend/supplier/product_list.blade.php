...
@section('page_css')
    <link property='stylesheet' href="{!! asset('assets/frontend/css/search-products.css',is_secured()) !!}" rel="stylesheet">
  @endsection
@section('content')
	<br>
<div class="row">
			...
			@foreach($products as $product)
				<div style="width: 100%;" class="list_product col-xs-12 ">
					<div class="col-xs-7 col-md-2 col-sm-4 padding_0">
						...
						<img style="height:150px;width:100%" class="img-thumbnail" src="{{ URL::to('uploads',(isset($pro_img->image)) ? $pro_img->image : 'no_image.jpg',null,is_secured()) }}" alt="" />
						@else
							@if($product->bdtdcProductToCategory)
								...
								<img style="height:150px;width:100%" class="img-thumbnail" src="{{ URL::to('bdtdc-product-image/'.trim($product->bdtdcProductToCategory->pro_parent_cat->name).'/'.trim($product->bdtdcProductToCategory->bdtdcCategory->name),(isset($pro_img_new->image)) ? $pro_img_new->image : 'no_image.jpg',null,is_secured()) }}" alt="" />
								@else
								<img style="height:150px;width:100%" class="img-thumbnail" src="{{ URL::to('uploads/no-image.jpg',null,is_secured()) }}" alt="" />
								@endif
							@else
							<img style="height:150px;width:100%" class="img-thumbnail" src="{{ URL::to('uploads/no-image.jpg',null,is_secured()) }}" alt="" />
							@endif
					...
					<div class="col-xs-6 col-md-6 col-sm-8">
						<div class="col-xs-12 padding_0">
							...
							{{ URL::to('product-details/'.preg_replace('/[^A-Za-z0-9\.-]/', '-',$product->product_name->name).'='.mt_rand(100000000, 999999999).$product->id,null,is_secured()) }}
							@else
							{{ URL::to('product-details/'.preg_replace('/[^A-Za-z0-9\.-]/', '-','no name').'='.mt_rand(100000000, 999999999).$product->id,null,is_secured()) }}
